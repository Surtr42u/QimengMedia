...
| `AuthorImportUseCase` | 作者导入 | `importAuthorsFromText`、`findMatchingMediaLight`、`rebuildAssociationsFromBlocks`（统一重建入口：从全部已导入 TXT 重建作者关联，被导入/刷新/删除/扫描四个场景复用，避免跨 TXT 同名作者关联被单 TXT 删旧插新覆盖）、`rematchAllTxtImports`（扫描后调用，内部委托 rebuild）、`rematchSingleTxtImport`（刷新降级，内部委托 rebuild）、`saveTxtUri`、`loadTxtUri`、`removeImportedTxtBlocks`、`removeImportedTxtUri` |
