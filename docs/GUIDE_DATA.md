...
| `personal_prefs_export.json` | 已实现 | 个人偏好数据导出（收藏+点赞+标签+统计） |
...
通过 ProfileFragment 数据管理弹层"导出偏好数据"触发，导出为两个文件：

| 文件 | 格式 | 用途 |
|---|---|---|
| `personal_prefs_export.json` | JSON | 给新 App 的迁移代码读取，包含全部数据 |
| `personal_prefs_report.txt` | 纯中文文本 | 给用户自己看，人类可读的偏好报告 |

### JSON 文件（给程序用）

`data` 区域包含全部数据（无数量限制），迁移代码读取此文件：
...
| `viewStats` | Room `ViewStatsEntity` | 观看/播放次数 + 浏览时长（全部） |
| `authors` | Room `AuthorEntity` + `AuthorMediaCrossRef` | 作者及关联文件列表（全部） |

### 文本报告（给用户看）

纯中文文本，包含以下章节：
- 【总览】收藏数、标签数、作者数、总浏览时长等
- 【点赞最多的文件 Top 20】
- 【观看最多的文件 Top 20】
- 【收藏的文件】全部列出
- 【使用最多的标签 Top 20】
- 【偏好度最高的作者 Top 20】（含标签关联）
- 【所有标签】全部列出（含关联文件数）
- 【所有作者】全部列出（含偏好度评分和标签关联）

作者偏好度评分：`preferenceScore` = 该作者所有文件的 (viewCount + playCount) 之和 + 每个收藏文件 +5 + 每个文件的 likeCount 之和

### 作者-标签关联

- JSON 中每个作者对象增加 `tags` 字段：`{"tag1": 3, "tag2": 1}`（标签名→该作者文件中出现次数）
- COS 偏好导出 JSON 中 `authorTags` 字段：`[{"authorName": "xxx", "tags": {...}}]`
- 文本报告中每个作者行后附标签信息：`标签：tag1(3), tag2(1)`，无标签时不显示
