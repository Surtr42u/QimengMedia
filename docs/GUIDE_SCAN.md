...
- `MediaStoreObserver`：基于 ContentObserver 监听 MediaStore 变化，自动增量更新已注册的常规扫描目录。5 秒防抖（v1.7：原 2 秒）。**只监听常规目录，不监听 COS 目录。**
...
- 收到变化通知后 5 秒防抖（v1.7：原 2 秒，延长以合并更多变更通知，减少频繁拍照场景下的扫描开销）
...
2. 从文件路径中解析出作者名和作品名（COS 路径解析算法，见下）
3. MediaStore 查不到时回退到 SAF 一次递归扫描整个根目录（`scanTreeFast`），从 SAF URI 路径解析作者/作品名
4. 数据库写入分批（每批 500 条），避免长时间锁表导致 UI 卡顿
5. `autoRefreshAllSources` 对 COS 目录有 5 分钟防抖间隔，避免频繁扫描
...
不管：数据实体定义（见 GUIDE_DATA.md）、备份流程（见 GUIDE_BACKUP.md）、UI 交互（见 GUIDE_UI.md）
