...
| `app/src/main/java/com/qimeng/media/ui/author/AuthorListFragment.kt` | 作者列表（全部/默认/COS胶囊筛选+排序） |
...
| 页面 | 左侧芯片 | 右侧芯片 | 说明 |
|---|---|---|---|
| 全部页 | 分区 (N) / 作品 (N) / 角色 (N) / 类型 (N) | — | 芯片文字带选项数量（不含"全部"和"收起"）；"类型"芯片展开后显示"全部/图片/视频/动图"药丸；"分区"芯片展示"默认 (N) / COS (M)"药丸，可折叠 |
...
| 作者管理 | 全部 (N) / 默认 (N) / COS (N) | 排序 ▾ | 胶囊切换筛选作者类型，排序对所有分类生效 |

**设计原则**：
- 芯片文字显示选项数量（如"作品 (12)"），数量不含"全部"和"收起"药丸
- "分区"芯片统一用于切换"默认/COS"分区，带数量显示和折叠功能
...
- 芯片栏：分区 / 作品 / 角色 / 类型
- **分区模式**：直接列出各数据源分区药丸（「默认 (N)」普通文件 + 「COS (M)」COS文件），点击切换选中状态（与出处/角色模式一致）；默认「默认」高亮显示全部非COS文件，选中COS后只显示COS文件；以后可扩展更多分区
...
- 数据管理行打开底部弹层，提供扫描目录、COS目录、TXT导入作者、数据备份、缓存与同步。
- 扫描目录：点击后弹出子弹层，上方显示已添加目录列表（每行显示目录名+删除按钮），下方"+ 选择文件夹"添加新目录。
...
- 芯片栏：分区 / 作品 / 角色 / 类型
- **全部模式**：平铺显示所有浏览历史文件，药丸区域显示"全部/COS"数据类型药丸
...
- COS 模式空状态文本："没有COS浏览记录"

## 收藏页

- 芯片栏：分区 / 作品 / 角色 / 类型
- **全部模式**：药丸区域显示"全部/COS"数据类型药丸，点击切换 COS 模式
...
- 全部模式下药丸区域显示"全部/COS"数据类型药丸（FlowLayout），扁平芯片风格（30dp高、选中bg_capsule_primary、未选中无背景），与出处/角色药丸样式一致
- 默认"全部"选中（高亮），点击"COS"切换到 COS 模式
- COS 模式下：
