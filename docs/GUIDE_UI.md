...
| `app/src/main/java/com/qimeng/media/ui/detail/ZoomImageView.kt` | 自定义缩放 ImageView（**智能分层渲染**：`applyOptimalLayerType()` 按图尺寸选层，GIF→`LAYER_TYPE_HARDWARE`，普通图长边≤4096（硬件渲染安全阈值）→`LAYER_TYPE_HARDWARE`（GPU直渲），>4096→`LAYER_TYPE_SOFTWARE`（保正确性，超大图GPU驱动无法正确应用matrix缩放）；手势期间用 `shouldUseHardwareForCurrent()` 同阈值判断；`lastDrawableWidth/Height` 记录实际尺寸防止Coil降采样导致matrix错乱；缩放范围 0.5x~5x，双击放大1.8x，近1.0的scale factor跳过减少无效重绘） |
...
| `app/src/main/java/com/qimeng/media/core/GpuInfo.kt` | GPU 纹理上限探测（EGL14 创建临时 context 查 `GL_MAX_TEXTURE_SIZE`，供 ZoomImageView 智能分层 + ImageLoader maxBitmapSize 配置用；探测失败回退 4096） |
...
- 图片（含 GIF 动图）：`ZoomImageView`，`showMediaAt()` 中 `MediaType.IMAGE` 和 `MediaType.ANIMATED_IMAGE` 均走图片分支（隐藏视频播放器 UI），GIF 动图使用 `AnimatedImageDecoder` 加载动画帧；使用 Coil `size(Size.ORIGINAL)` 加载原图（**设计决策：详情页始终显示完整原图，不降采样**，用户点击查看的就是原始分辨率图片），淡入淡出滑动；始终 edge-to-edge 全屏布局，系统栏显隐不触发布局变化，避免图片重新居中；**智能分层渲染**（2026-06-20 优化，2026-06-21 修复超大图渲染阈值，2026-06-23 去重）：`ZoomImageView.setImageDrawable` 调用 `applyOptimalLayerType()` 按图尺寸智能选层（`setImageBitmap` 不再单独调用，由 `super.setImageBitmap` 链式触发的 `setImageDrawable` 统一负责，消除同图二次判断冗余）——GIF（`AnimatedImageDrawable`）→ `LAYER_TYPE_HARDWARE`（动画依赖硬件刷新帧）；普通图长边 ≤ **硬件渲染安全阈值 4096**（`HARDWARE_RENDER_SAFE_SIZE`）→ `LAYER_TYPE_HARDWARE`（GPU 直渲，4096 图绘制从 ~50ms 降到 ~5ms，消除来回切大图卡顿）；普通图长边 > 4096 → `LAYER_TYPE_SOFTWARE`（CPU 软件渲染，正确应用 MATRIX 缩放）。**⚠️ 阈值选择根因**（2026-06-21 修复）：原方案用 `GpuInfo.maxTextureSize()`（探测 `GL_MAX_TEXTURE_SIZE`，旗舰机实测可达 16384）作阈值，但这是 OpenGL 理论上限，厂商 GPU 驱动对超大 bitmap（如 8000x8000=256MB）无法正确应用 MATRIX 缩放——超大图走 HARDWARE 时 matrix 失效，图按原始像素从左上角绘制（表现为左上角小方块、不居中）。4096 是业界公认稳定值（Glide/Fresco 默认上限，Android Canvas 软件层 Skia MAXMIMUM_BITMAP_SIZE=32766 远大于此不会崩），与 `GpuInfo` 探测失败回退值 `DEFAULT_MAX_TEXTURE_SIZE=4096` 一致。手势期间（双指缩放/拖拽）同样用 `shouldUseHardwareForCurrent()` **同 4096 阈值**判断，超大图手势时保持 SOFTWARE。不再使用 `Bitmap.Config.HARDWARE`（原 ≤4096px 用 HARDWARE 的方案会导致 HARDWARE Bitmap 在 SOFTWARE 层上无法绘制，大图白屏卡死）；**两个独立阈值需区分**：①渲染安全阈值 4096（`HARDWARE_RENDER_SAFE_SIZE`，HARDWARE/SOFTWARE 分层判断，保渲染正确性）②Coil maxBitmapSize = `gpuMax`（`GpuInfo.maxTextureSize()`，解码降采样上限，放开让 8192 解原图）。两者独立，改其一不影响另一个；**Coil maxBitmapSize 放开**（2026-06-20）：Coil3 默认 `maxBitmapSize=4096` 会把超大图降采样（8192→4096），偏离"始终原图"决策。`QimengApplication` ImageLoader 配置 `.maxBitmapSize(Size(gpuMax, gpuMax))` 放开到 GPU 上限，让 8192/7680 等超大图显示原图。代价是内存上升（超大图原图 ARGB_8888 ~150MB/张，浏览多张超大图 Native Heap 峰值可达 ~900MB），靠 Coil LRU 35% + onTrimMemory + 超大图预渲染收紧控制；**超大图 SOFTWARE 渲染 trade-off**（2026-06-21）：超大图（>4096）走 SOFTWARE 比 HARDWARE 慢（CPU 逐像素处理，8000x8000 量级可能 100ms+），但保证 matrix 缩放正确、显示位置正常。保正确性优先于渲染速度，这是已知 trade-off；**缩放范围**：0.5x~5x，双击放大3x；**性能优化**：近1.0的scale factor跳过减少无效重绘，`requestDisallowInterceptTouchEvent` 手势期间只调用一次；退出详情页时取消所有加载/预加载请求、释放ImageView drawable，确保全尺寸bitmap被回收
- 视频（仅 `MediaType.VIDEO`）：未播放时用 `ZoomImageView` 显示视频帧预览（透明背景，透出主题色，与图片分支一致；优先使用 `MediaMetadataRetriever` 提取首帧，失败时回退到 Coil VideoFrameDecoder），中央显示 ▶ 播放按钮；单击预览区直接启动播放（进入 BiliPlayerView），横滑浏览其他文件。播放时 BiliPlayerView 接管触摸
...
- 预加载：`preloadAround()` 双向预加载，基础 1 前 2 后（阅读 D 时预渲染 C/D/E/F），内存充裕时 +1 每侧（2 前 3 后）；**超大图收紧**（2026-06-20）：当前图长边 > 4096（`HUGE_IMAGE_THRESHOLD`）时收紧到 1 前 1 后，避免超大图原图（~150MB/张）预渲染过多导致 OOM；每次切换取消所有旧预加载并重新预加载范围内所有项（Coil 内部缓存去重，已命中的项预加载会直接跳过，不会重复解码）；退出详情页时统一取消预加载；**placeholder 保留画面**：所有 Coil load 设置 `placeholder(当前drawable)`，加载期间保留旧画面不清空，根治来回滑动闪白；**Coil 缓存上限**：堆的 35%；**主线程同步缓存检查**（2026-06-19 优化）：`showImage()` 先在主线程同步检查 Coil 内存缓存（LruCache 线程安全），命中则直接 `loadOriginalImageWithCoil` 走 Coil load，跳过 `LargeImageDecoder` 的 `withContext(Dispatchers.IO)` 线程切换开销（40MB+ 超大图场景下省 13-34ms）；未命中才异步走 `LargeImageDecoder` 解码；**大图解码优化**（`LargeImageDecoder`，绕过 Coil Decoder 链的自定义解码器）：内部解码逐级回退——Coil 内存缓存命中（仅检查是否存在，不取出 Bitmap 对象——避免缓存驱逐时 recycle 导致 ImageView 绘制已回收 bitmap 崩溃；命中时返回 null 让 Fragment 走 Coil load 路径，Coil 命中内存缓存零延迟且正确管理 bitmap 生命周期）→ PNG 用 `SpngDecoder`（libspng，ARM NEON 优化，比系统 libpng 快 2-3 倍）→ `BitmapFactory.decodeFileDescriptor`（跳过 InputStream 中间层，比 Coil 的 decodeStream 快 ~10-20%）→ `BitmapFactory.decodeStream`（异常回退）；`LargeImageDecoder` 全部失败则由 Fragment 回退到 Coil 标准流程；解码成功后写入 Coil 内存缓存供后续访问零延迟；并发解码信号量限制（大图同时解码不超过 2 张，防止内存峰值）；预加载使用 Coil 标准流程（`size(Size.ORIGINAL)` + `memoryCacheKey(recordKey)`），自动利用内存/磁盘缓存；**视频帧取首帧**：详情页视频预览和预加载统一用 `videoFrameMillis(0)` 取首帧，避免 3 秒帧 seek 开销（详见 GUIDE_DEBUG.md 性能约束）；**视频预览缓存命中**（2026-06-22 优化）：`showVideo()` 与 `showImage()` 同策略，先在主线程同步检查 Coil 内存缓存（预加载已将视频帧写入缓存时命中），命中则直接 `loadVideoFrameWithCoil` 走 Coil load 零延迟显示，跳过 `MediaMetadataRetriever` 的 IO 线程打开+解码开销（消除"图片滑到视频卡一下"）；未命中才异步走 `loadVideoFrame`（MediaMetadataRetriever）；**滑动防抖**：`moveBy()` 设置 `isTransitioning` 后延迟 200ms 重置，覆盖异步加载窗口防止快速连续滑动竞态；**切换不闪白**：`showImage()`/`showVideo()` 不立即清空当前画面，保留旧图直到新内容加载完成；**bitmap 回收防护**：所有 `setImageBitmap` 调用前检查 `!bitmap.isRecycled`，已回收 bitmap 自动回退到 Coil load；**诊断日志**（2026-06-20 新增，消除 GUIDE_DEBUG 观察盲区）：`showImage()` 缓存命中分支打 `Detail: showImage: 命中内存缓存 key=xxx`，未命中分支打 `缓存未命中，异步解码` + 解码完成 `解码完成 key=xxx 耗时=Nms 尺寸=WxH`（耗时用 `SystemClock.uptimeMillis()` 包住 `decodeCurrentImage`）/ 回退 `回退Coil key=xxx 耗时=Nms`；`preloadAround()` 入口打 `preload: 范围[start..end] 当前=index 内存充裕=B 入队=N`；`LargeImageDecoder` 各回退路径打 `LargeImage: libspng/fileDescriptor 解码成功|失败`、`回退到 Coil 标准流程`。日志样例与命中率诊断方法详见 GUIDE_DEBUG.md「缩略图/详情页不显示诊断」
