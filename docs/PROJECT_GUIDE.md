...
- 已完成真机性能监测三项开销修复（2026-06-22）：基于真机性能监测（AppLog 文件日志 + `dumpsys gfxinfo/meminfo` + `am start -W`，冷启动 657ms / 列表滚动 Janky < 2% / 详情页缓存命中率 100%，整体性能优秀）发现并修复三项可优化开销：① `AllFilesFragment` collect 块重复 render（`computeSourceGroups()` 空壳委托与 `render()` 连调，同一 fingerprint render 两次，全部页首次进入 415ms→210ms）；② `SourceMatcher.matchAll` 结果按 fileName 缓存（消除批量遍历重复匹配，charMiss 日志采样每 50 次记 1 条防刷屏）；③ `ThumbnailCache.pregenerateThumbnails` 全量缓存命中时批量预过滤短路（跳过并发池调度，autoRefresh 触发的空遍历开销消除）。构建验证 `assembleDebug` + `testDebugUnitTest` 全过，详见 `GUIDE_CODE_MAINTENANCE.md`「性能优化（2026-06-22）」。
- 已完成备份恢复提示 + 空库覆盖防护（2026-06-22）：① 恢复提示——用户在数据管理弹层选择备份目录后，`BackupManager.peekBackupSummary`（只读，新增）检测 `app数据/` 子目录是否已有备份数据，有则弹 `AlertDialog` 提示"X 位作者 / Y 个标签 / Z 条统计，是否导入恢复"，用户确认后调 `importFromDirectory` 恢复；同一目录只提示一次（独立 SharedPreferences `restore_hint_prefs` 记录，卸载重装后清空会重新提示）；② 空库覆盖防护——`AutoSyncUseCase` 三个写入点（`syncAppDataOnly`/`triggerFullSync`/`triggerManualSync`）调用 BackupManager 前用 `MediaFileDao.hasAny()`（EXISTS+LIMIT 1，新增）检测 `media_files` 是否为空，空库时跳过写入并置 `restoreSuggestion` StateFlow 为 true，数据备份弹层显示"⚠ 本地数据为空，已暂停自动同步，建议先导入恢复"提示行 + "导入恢复"按钮；③ `FORMAT_SPEC.md`/`DATA_MIGRATION_SPEC.md` 新增「恢复流程」章节，说明导入语义和空库防护，供其他 App 开发者参考实现"写回恢复"。设计目的：让备份数据成为长期个人用户画像，支持跨 App/跨设备/跨时间延续，避免卸载重装后选了旧目录却不合并数据、或空数据库反向覆盖旧备份导致数据永久丢失。详见 `GUIDE_BACKUP.md`「数据管理功能」「写入原则」「导入原则」。
- 已修复两个导入数据丢失 bug（2026-06-22）：① `importTags` 漏读 `mediaTags` 数组——导出端 `exportTags` 写了 `tags` + `mediaTags` 两个数组，但 `importTags` 只读 `tags`，完全没读 `mediaTags`，导致所有文件-标签关联（`MediaTagCrossRef`）导入后整体丢失；修复后 `importTags` 先导入 tag 定义，再读 `mediaTags` 按 `tagName` 查本地真实 `tagId`（避开跨设备 tagId 漂移），`recordKey` 本地不存在则跳过（与 `importAuthors` 一致，防孤儿关联）；② `exportScanSources`/`importScanSources` 漏写 `isCosDirectory`——导出端没写该字段，导入端构造 `ScanSourceEntity` 也没传（用默认 false），导致 COS 扫描源恢复后身份丢失、COS 目录变常规目录；修复后两端均补 `isCosDirectory`，旧备份缺失时导入默认 false（向后兼容）。合并策略暂不改动（大多数场景导入时本地为空库，不存在合并冲突）；文件改名迁移暂缓（记入 `DEVELOPMENT_PLAN.md` 待办）。
